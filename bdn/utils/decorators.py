        ...
        if needs_origin:
            if self.is_method:
                def wrapped(inst, *args, **kwargs):
                    return fn(inst, origin, *args, **kwargs)
            else:
                def wrapped(*args, **kwargs):
                    return fn(origin, *args, **kwargs)
...
def _check_decoratable(origin, install=True):
    if not hasattr(origin, '_decoratable_obj'):
        is_method = False
        if inspect.isfunction(origin) and sys.version_info.major > 2:
            try:
                cls = getattr(
                    inspect.getmodule(origin),
                    origin.__qualname__.split(
                        '.<locals>', 1)[0].rsplit('.', 1)[0])
                is_method = True
            except AttributeError:
                pass
        else:
            is_method = inspect.ismethod(origin)
            if is_method:
                cls = origin.im_class

        if is_method and not hasattr(origin, '_decoratable_obj'):
...
def _decorate_result(origin, needs_params=False):
    ...
    def decorator(fn):
        origin._decoratable_obj._decorate(
            fn, DecoratableObject.MODE_RESULT, needs_params=needs_params)
...
def _decorate_with(fn):
    def decorator(origin):
        origin = _check_decoratable(origin)
        origin._decoratable_obj._decorate(
            fn, DecoratableObject.MODE_OVERRIDE, needs_origin=True)
        return origin
    return decorator
